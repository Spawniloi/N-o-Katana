...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if distance_to_player <= self.attack_range:
            if player.attacking:
                self.hit()  # Supprimer l'ennemi après avoir été attaqué avec succès
            else:
                self.attack()  # Déclencher l'attaque de l'ennemi sur le joueur
                player.take_damage(self.damage)  # Réduire la santé du joueur
...
enemies.add(Enemy(200, 200, 2, attack_range=100))  # Utilisation d'un argument nommé attack_range
...
while True:
    ...
    # Dessiner la vie du joueur en haut à droite de l'écran
    player_health_text = font.render("Health: " + str(player.health), True, BLACK)
    screen.blit(player_health_text, (SCREEN_WIDTH - player_health_text.get_width() - 10, 10))
